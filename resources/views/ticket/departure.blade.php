...
@media (max-width: 768px) {
  ...
  <div class="container ">
        <h1 class="mx-2 mx-md-5 mt-5 mb-5 text-start fs-3 fw-normal  ">
            <span style="color: #374294 ">{{ $departureFlight->departureAirport->city }}</span> to <span style="color: #374294">{{ $departureFlight->arrivalAirport->city }}</span> on  {{ $departureFlight->departure_time->format('l, F j') }}
        </h1>

        <svg xmlns="http://www.w3.org/2000/svg" style="position: absolute; top:250px; left:168px" class="mx-auto " width="45" height="45" viewBox="0 0 55 55" fill="none">
        ...
        <div class=" departure row g-3 mx-2 mx-md-5 border shadow-sm border-start-0 rounded-3   ">
                ...
                {{-- @if(isset($departureFlightId) && isset($classType))
                <p>Departure Flight ID: {{ $departureFlightId }}</p>
...
{{--
                ...
                @foreach($returnFlights as $returnFlight)
                    <div class="row text-center shadow-sm rounded-4 m-3 mx-5 p-2 bg-light">
                        <!-- Departure Details (Return Flight Departure) -->
                        <div class="col-md-1 d-flex flex-column align-items-center mt-3">
                            <h6>{{ $returnFlight->departure_time->format('H:i') }}</h6>
                            <p>{{ $returnFlight->departureAirport->code }}</p>
                            <p style="color: #374294">{{ $returnFlight->departureAirport->city }}</p>
                        </div>
                        ...
                        <div class="col-md-1 d-flex flex-column align-items-center mt-3">
                            ...
                            <p>{{ $returnFlight->arrivalAirport->code }}</p>
                            <p style="color: #374294">{{ $returnFlight->arrivalAirport->city }}</p>
                        </div>
                        ...
                        <!-- Flight Class and Select Button -->
                        <div class="col-md-2 pt-2 pb-1 me-2 mx-4 border rounded-4 shadow-sm d-flex flex-column align-items-center custom-card">
                            <p class="mt-2">Class Type</p>
                            <button onclick="window.location.href='{{ route('select.return', ['returnFlightId' => $returnFlight->id, 'class' => 'economy']) }}'" class="btn btn-primary">Select this flight</button>
                        </div>
                    </div>
                @endforeach
            @else
            ...
            @endif --}}
                ...
                <hr class="mx-5 mt-5">
                {{-- <div class="container">
                    <h1 style="width: 40rem; font-size: 50px; font-style: normal; font-weight: 400; line-height: normal; margin-top:70px; margin-bottom:60px" class="mx-5">
                        Select your return flight from <span style="color: #374294">{{ $departureCity }}</span> to <span style="color: #374294">{{ $arrivalCity }}</span>
                    </h1>
                </div> --}}
                <div class="container">
                    ...
                    font-style: normal;
                    font-weight: 400;
                    line-height: normal; margin-top:100px ;margin-bottom:60px"  class="mx-5  ">Select your return flight from <span style="color: #374294">{{ $departureCity }}</span>  to <span style="color: #374294">{{ $arrivalCity }}</span>
                     </h1>
...
<div class=" row mt-5">
    ...
    {{-- Header and count for return flights --}}

<div class="mx-5 my-3 mt-5 d-flex flex-column align-items-left">
    <h6 style="color: #374294">{{ $returnFlights->count() }} return flight results</h6>
    <p>Fares displayed are for all passengers.</p>
</div>


{{-- Loop through each return flight and display its details --}}
{{-- @foreach($returnFlights as $returnFlight)
<div class="row text-center shadow-sm rounded-4 m-3 mx-5 p-2 bg-light">
    <!-- Departure Details -->
    <div class="col-md-1 d-flex flex-column align-items-center mt-3">
        <h6>{{ $returnFlight->departure_time->format('H:i') }}</h6>
        <p>{{ $returnFlight->departureAirport->code  }}</p>
        <h6 style="color: #374294">{{$returnFlight->departureAirport->city}}</h6>
    </div>
    ...
    <!-- Return Flight Arrival Details -->
        ...
        <div class="col-md-2 d-flex flex-column align-items-center mt-3">
            <h6>{{ $returnFlight->arrival_time->format('H:i') }}</h6>
            <p>{{  $returnFlight->arrivalAirport->code  }}</p>
            <h6 style="color: #374294">{{ $returnFlight->arrivalAirport->city }}</h6>
        </div>
...
<!-- Offcanvas Details for Return Flight -->
<div class="offcanvas offcanvas-end" id="offcanvasReturnDetails{{ $loop->index }}" aria-labelledby="offcanvasReturnDetailsLabel{{ $loop->index }}">
    <div class="offcanvas-header">
        <h5 style="color: #374294; margin-top:20px" id="offcanvasReturnDetailsLabel{{ $loop->index }}">Return Flight Details</h5>
        <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body">
        <h4 style="color: #374294; margin-top:70px ;" class="offcanvas-title" id="offcanvasWithBothOptionsLabel">Return Ticket</h4>
        <h5 style="color: rgb(197, 174, 57); margin-top:70px" class="text-start mb-3">{{ $returnFlight->departureAirport->city }} to {{ $returnFlight->arrivalAirport->city }}</h5>
        <p class="text-start fw-light ">{{ $returnFlight->departure_time->format('D, d M Y') }}</p>
        <div class="d-flex">
            <p>{{ $returnFlight->departure_time->format('H:i') }}</p>
            <p style="font-size: 15px" class="text-start mx-3">{{ $returnFlight->departureAirport->city }}<br>{{ $returnFlight->departureAirport->name }} ({{ $returnFlight->departureAirport->code }})</p>
        </div>
        <div style="height: 50px; width: 1px; background-color: black; margin-bottom:30px" class="vertical-line mx-3"></div>
        <div class="d-flex justify-content-between">
            <p>{{ $returnFlight->arrival_time->format('H:i') }}</p>
            <p style="font-size: 15px" class="text-start me-3">{{ $returnFlight->arrivalAirport->city }}<br>{{ $returnFlight->arrivalAirport->name }} ({{ $returnFlight->arrivalAirport->code }})</p>
        </div>
        <hr class="mx-0 my-0">
    </div>
</div>
    ...
    <div class="offcanvas-body">
        ...
        <div style="  height: 100px;
            ...
            <div class="d-flex ">
                ...
                <p>{{ $returnFlight->arrival_time->format('H:i') }}</p>
                <p style="font-size: 15px" class="text-start mx-3">{{ $returnFlight->arrivalAirport->city }}  <br> {{ $returnFlight->arrivalAirport->name }} ({{ $returnFlight->arrivalAirport->code }})</p>
             ...
             <!-- Economy Class -->
 <a href="{{ route('return-flight-selection', ['returnFlightId' => $returnFlight->id, 'classType' => 'economy']) }}"class="col-md-2 pt-2 pb-1 me-2 mx-4 border rounded-4 shadow-sm d-flex flex-column align-items-center custom-card text-decoration-none text-dark">
    <i class="fas fa-plane fa-2x"></i>
    <p class="mt-2">Single Economy</p>
    <h3  style="color: #287ba7;">${{ $returnFlight->price_economy }}</h3>
</a>

<a href="{{ route('return-flight-selection', ['returnFlightId' => $returnFlight->id, 'classType' => 'business']) }}"class="col-md-2 pt-2 pb-1 border rounded-4 shadow-sm d-flex flex-column align-items-center custom-card text-decoration-none text-dark">
    <i class="fas fa-business-time fa-2x"></i>
    <p class="mt-2">Single Business</p>
    <h3 style="color: #c01b1b;">${{ $returnFlight->price_business }}</h3>
</a>
...
<div class="flight-ticket row shadow-sm rounded-4 border-1 m-3 mx-5 p-3 border bg-light">
    <div class="col-12 col-md-3 d-flex flight-info">
        <div class="time-location ">
            <h6 class="departure-time">{{ $returnFlight->departure_time->format('H:i') }}</h6>
            <p class="badge badge-primary">{{ $returnFlight->departureAirport->code }}</p>
            <h6>{{ $returnFlight->departureAirport->city }}</h6>
        </div>
        <div class="flight-icon  ">
            <i class="fas fa-plane-departure fa-2x"></i>
        </div>
        <div class="time-location ">
            <h6 class="arrival-time">{{ $returnFlight->arrival_time->format('H:i') }}</h6>
            <p class="badge badge-primary">{{ $returnFlight->arrivalAirport->code }}</p>
            <h6>{{ $returnFlight->arrivalAirport->city }}</h6>
        </div>
    </div>
...
<div>
    ...
    <h6>{{ $returnFlight->remainingSeats() }}</h6>
</div>
</div>
<!-- Offcanvas Details -->
<div class="offcanvas offcanvas-end" id="offcanvasDetails{{ $loop->index }}" aria-labelledby="offcanvasDetailsLabel{{ $loop->index }}">
...
<div class="offcanvas-body">
     ...
     <div class="d-flex ">
            ...
            <p >{{ $returnFlight->departure_time->format('H:i') }}</p>
            <p style="font-size: 15px" class="text-start mx-3 ">{{ $returnFlight->departureAirport->city }}<br>{{ $returnFlight->departureAirport->name }} ({{ $returnFlight->arrivalAirport->code }})</p>
    ...
    <div class="col-sm-4 text-center flight-details  mt-2 mx-3">
        <a href="{{ route('return-flight-selection', ['returnFlightId' => $returnFlight->id, 'classType' => 'economy']) }}" class="btn  btn-primary rounded-4">Single Economy - ${{ $returnFlight->price_economy }}</a>
        <a href="{{ route('return-flight-selection', ['returnFlightId' => $returnFlight->id, 'classType' => 'business']) }}" class="btn btn-secondary rounded-4">Single Business - ${{ $returnFlight->price_business }}</a>
    </div>
</div>
@endforeach
    {{-- <div class="row text-center  shadow-sm rounded-4 m-3  mx-5 p-2 bg-light">
...
<div class="col-md-2 p-3 py-4 border rounded-4 me-md-0 shadow-sm d-flex flex-column ms-3 text-decoration-none text-dark items-center custom-card" data-bs-toggle="collapse" href="#economyCollapse" role="button" aria-expanded="false" aria-controls="economyCollapse">
    <p>Economy</p>
    <h3>$1,382</h3>
</div>

<div class="collapse" id="economyCollapse">
    <!-- Your collapsible content goes here -->
    <p>Collapsible content for Economy</p>
            ...
            <div class="container">
                <div class="row mx-0  ">
                  <div  class="card rounded-4   col-md-4  px-0">
                    ...
                    <button class="border-0 rounded-bottom-4 mt-2 text-center custom-hover" onclick="window.location.href='{{ url('return') }}'">Select this flight</button>
